...
if D_check != "事件":
    ...
    exit()
J_check = str(ws.cell(row=3, column=10).value)
if J_check != "事件标识EVENT_ID(规范定义)":
    print "J列有异常，请查看表格是否变更"
    exit()
...
for i in range(5, ws.max_row):
    ...
    J_column = str(ws.cell(row=i, column=10).value)
    ...
    exegesis = "// " + B_column + "_" + C_column + "_" + D_column + "\n"  # 注释拼接

    java_define = prefix + J_column + " = " + L_column + ";" + "\n\n"  # 拼接java常量定义

    print(exegesis + java_define)
    output_content = output_content + exegesis + java_define

with open(output_file_path + "/DataReport.java", 'w') as f:
